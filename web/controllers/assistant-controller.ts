  ...
  try {
    ...
    const generateForGenre = async (genre: Type) => {
      ...
      const results = await Promise.all(
        topicResp.topics.map(async (topic) => {
          const topicResults = [];
          topicResults.push(
            await queue(genre, topicResp.genre, topicResp.subgenre, topic, "A1")
          );
          topicResults.push(
            await queue(genre, topicResp.genre, topicResp.subgenre, topic, "A2")
          );
          topicResults.push(
            await queue(genre, topicResp.genre, topicResp.subgenre, topic, "B1")
          );
          topicResults.push(
            await queue(genre, topicResp.genre, topicResp.subgenre, topic, "B2")
          );
          topicResults.push(
            await queue(genre, topicResp.genre, topicResp.subgenre, topic, "C1")
          );
          topicResults.push(
            await queue(genre, topicResp.genre, topicResp.subgenre, topic, "C2")
          );
          return topicResults;
        })
      );
    ...
    const failedResults = combinedResults.filter(
...
async function queue(
  ...
  try {
    ...
    const articleResp = await retry(() =>
      generateArticle(type, genre, subgenre, topic, level)
    );
    console.log("GENERATED ARTICLE");
    const mcResp = await retry(() =>
      generateMC(
        articleResp.cefrLevel,
        articleResp.type,
        articleResp.genre,
        articleResp.subgenre,
        articleResp.passage,
        articleResp.title,
        articleResp.summary,
        articleResp.imageDesc
      )
    );
    console.log("GENERATED MC");
    const saResp = await retry(() =>
      generateSA(
        articleResp.cefrLevel,
        articleResp.type,
        articleResp.genre,
    ...
    const audioResp = await retry(() =>
      generateAudio(articleResp.passage, articleDoc.id)
    );
    console.log("GENERATED AUDIO");
    const imageResp = await retry(() =>
      generateImage(articleResp.imageDesc, articleDoc.id)
    );
...
export async function generateArticle(
  ...
  const cefrLevelPrompts: CefrLevelPromptType[] = JSON.parse(
    fs.readFileSync(
      path.join(process.cwd(), "data", "cefr-level-prompts.json"),
      "utf-8"
    )
  );

  const levelConfig = cefrLevelPrompts
    .find((item) => item.levels.some((lvl) => lvl.level === level))
...
async function generateContent<T>(
  ...
  const cefrLevelPrompts = JSON.parse(
    fs.readFileSync(path.join(process.cwd(), "data", promptFile), "utf-8")
  );
  const { system_prompt: systemPrompt, user_prompt: baseUserPrompt } =
    cefrLevelPrompts[type][cefrLevelReformatted];
  ...
  try {
    ...
    throw new Error(
      `Failed to generate content step: ${promptFile}, ${error},`
    );
  }
}
