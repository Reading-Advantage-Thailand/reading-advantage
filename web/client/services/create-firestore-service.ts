...
const createFirestoreService = <T extends DocumentData>(collectionPath: string) => {
    ...
    return {
        ...
        fetchAllDocs: async (filter?: { select?: string[] }, headers?: ReadonlyHeaders) => {
            const selectParams = filter?.select ? encodeURIComponent(JSON.stringify(filter.select)) : "";
            const url = selectParams ? `${apiUrl}?select=${selectParams}` : apiUrl;
            return fetchData<{ length: number; data: T[] }>(url, { method: "GET", headers: headers, cache: "no-store" }, {
                title: "Failed to fetch documents",
            });
        },

        fetchFilteredDocs: async (filter: { orderBy: string, startAt: number, limit: number, select: string[] }, headers?: ReadonlyHeaders) => {
