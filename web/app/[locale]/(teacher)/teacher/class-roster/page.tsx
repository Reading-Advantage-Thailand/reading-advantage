...
export default async function RosterPage(params: {
 ...
 const userRecordsMatch = results.results.filter((record: any) => record.userId === studentId);

if (userRecordsMatch.length > 0) {
  const sortedData = results.results.sort((a: any, b: any) => {
  ...
  const sortedLastActivityTimestamp = Object.entries(lastActivityTimestamp).sort(
