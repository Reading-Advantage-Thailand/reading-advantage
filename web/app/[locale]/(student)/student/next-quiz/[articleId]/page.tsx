...
export default async function ArticleQuizPage({ params }: { params: { articleId: string } }) {
    ...
    if (articleResult.message === 'Article not found') return <div>{t('articleNotFound')}</div>;
    if (articleResult.message === 'Insufficient level') return <div>{t('articleInsufficientLevel')}</div>;
    if (articleResult.message === 'Internal server error') return <div>Internal server error</div>;

    const articleRecord = await getArticleRecord(user.id, params.articleId);
    if (articleRecord.message === 'Insufficient level') return <div>{t('articleInsufficientLevel')}</div>;
    if (articleRecord.message === 'Internal server error') return <div>Internal server error</div>;
