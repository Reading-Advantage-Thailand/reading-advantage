...
async function getEnhancedGenreMetricsResponse(
  ...
  try {
    ...
    // Check authorization
    const authCheck = await checkEnhancedAuthorization(req.session, scope, scopeId);
    ...
    console.log(`[Controller] getEnhancedGenreMetrics - ${duration}ms - ${scope}:${scopeId}`);
  ...
  switch (scope) {
    case 'student':
      ...
      if (user.role === 'STUDENT' || user.role === 'USER') {
        if (user.id !== scopeId) {
          return { authorized: false, error: 'Students can only access their own metrics' };
        }
        ...
        if (!studentAccess) {
          return { authorized: false, error: 'Teacher does not have access to this student' };
        }
