...
const createFirestoreService = <T extends DocumentData>(collection: DBCollection, parent?: string) => {
    return {
        ...
        getAllDocs: (filter: { select: string[] }) => getAllDocs<T>(collection, filter, parent),
