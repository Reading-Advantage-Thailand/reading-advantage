...
//   {
//     accessorKey: "action",
//     header: () => <div className="captoliza">Action</div>,
// },
// ]
...
const columns: ColumnDef<{ id: string; }, any>[] = [
  ...
  },
  {
      ...
      // cell: ({ row }) => <div className="captoliza">{row.getValue("action")}</div>,
      cell: ({ row }) => (
        <DropdownMenu>
...
export default function MyStudents({userId, allStudent, allClassroom}: MyStudentProps) {
  ...
  const table = useReactTable({
    data: matchedStudents.map(studentId => ({ id: studentId, name: allStudent.find(student => student.id === studentId)?.name, email: allStudent.find(student => student.id === studentId)?.email})),
    columns,
