             ...
             <div className="flex flex-col justify-between h-full overflow-hidden">
               <div className="overflow-y-auto flex-1">
                 {messages.map((message, index) => (
                   <div
                     key={index}
                     className={`rounded-lg text-white break-words text-wrap m-2 p-2 ${
                       message.sender === "user"
               ...
               <div className="shrink-0 flex nt-2">
                 <input
                   type="text"
                   value={userInput}
...
/*
   return (
     <div className="fixed bottom-4 right-4 flex items-center space-x-2 z-50">
       ...
       {isOpen && (
         <div className="absolute bottom-14 right-0 w-96 h-96 bg-white border border-gray-300 p-4 shadow-xl rounded-lg overflow-y-auto ">
           <div className="overflow-y-auto flex-1 h-full">
             {messages.map((message, index) => (
               <div
                 key={index}
                 className={`rounded-lg text-white break-words text-wrap m-1 p-2 ${
                   message.sender === "user"
                     ? "bg-blue-500 self-end"
                     : "bg-gray-500 self-start"
                 }`}
