...
collectionRef.get()
  .then((snapshot) => {
    ...
    const oldSubgenre = article.subgenre;

    if (genre in genreMapping && oldSubgenre in genreMapping[genre] && articleType === 'nonfiction') {
       const newSubGenre = genreMapping[genre][oldSubgenre];
       doc.ref.update({ subgenre: newSubGenre})
         .then(() => {
